...
class VoidBots extends EventEmitter {
    ...
    async _webhookServer() {
        ...
        try {
            ...
            throw Error('[Voidbots] Error while requiring packages (localtunnel, express)')
        }
        const app = express(),
            port = 5600
        ...
        this.voteWebhook = {
            url: `${tunnel.url}/vote`,
            auth: this._createKey()
        }
        this._request(`/bot/votewebhook/${this.client.user.id}`, 'POST', {
            webhook_url: this.voteWebhook.url,
            webhook_auth: this.voteWebhook.auth
        });
        ...
        app.listen(port, async () => {
            console.log(`[Voidbots] Webhook server initialized`)
        });
    }
